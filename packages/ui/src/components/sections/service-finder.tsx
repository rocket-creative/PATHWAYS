...
export function ServiceFinder({ site = 'main' }: ServiceFinderProps) {
  ...
  const contactUrl = site === 'main' ? '/contact' : `${mainUrl}/contact`
  ...
  const getResults = (answers: string[]) => {
    ...
    if (intent === 'therapy' || focus === 'anxiety' || focus === 'relationships' || focus === 'trauma') {
      if (focus === 'trauma') {
        return {
          ...
          description: "We specialize in gentle, evidence-based approaches to help you process difficult experiences.",
          services: [
            { name: "EMDR Therapy", link: `${wisdomUrl}/services/emdr-therapy` },
            { name: "Trauma Therapy", link: `${wisdomUrl}/services/trauma-therapy` },
            { name: "Somatic Therapy", link: `${wisdomUrl}/services/somatic-therapy` },
          ],
      ...
      return {
        ...
        description: "Our licensed therapists are here to help you navigate life's challenges with compassion.",
        services: [
          { name: "Individual Therapy", link: `${wisdomUrl}/services/individual-therapy` },
          { name: "EMDR Therapy", link: `${wisdomUrl}/services/emdr-therapy` },
          { name: "Hypnotherapy", link: `${wisdomUrl}/services/hypnotherapy` },
        ],
    ...
    if (intent === 'wellness' || focus === 'physical' || focus === 'skin') {
      if (focus === 'skin') {
        return {
          ...
          description: "Reveal your natural radiance with our medical-grade skincare treatments.",
          services: [
            { name: "HydraFacial", link: `${wellnessUrl}/services/hydrafacial` },
            { name: "PRP Vampire Facial", link: `${wellnessUrl}/services/prp-vampire-facial` },
            { name: "Skincare Services", link: `${wellnessUrl}/services/skincare` },
          ],
      ...
      return {
        ...
        description: "Release tension, reduce pain, and restore your body's natural balance.",
        services: [
          { name: "Massage Therapy", link: `${wellnessUrl}/services/massage` },
          { name: "Acupuncture", link: `${wellnessUrl}/services/acupuncture` },
          { name: "Pain Management", link: `${wellnessUrl}/services/pain-management` },
        ],
    ...
    return {
      ...
      description: "You're in the right place. Pathways Within offers both therapy and wellness services.",
      services: [
        { name: "Individual Therapy", link: `${wisdomUrl}/services/individual-therapy` },
        { name: "Massage Therapy", link: `${wellnessUrl}/services/massage` },
        { name: "All Services", link: site === 'main' ? '/services' : `${mainUrl}/services` },
      ],
        ...
        <div className="mx-auto max-w-4xl">
          <div className="mb-12 text-center">
            ...
            <p className="text-lg text-[rgb(var(--color-text-light))]" style={{ lineHeight: 1.7 }}>
              {showResults ? results?.description : "Take our 30-second quiz to discover which services are right for you."}
            </p>
          </div>
            ...
            <>
              <div className="mb-10">
                <div className="h-1 w-full overflow-hidden rounded-full bg-[rgb(var(--border))]/30">
                  <div className="h-full rounded-full bg-[rgb(var(--color-green))] transition-all duration-500" style={{ width: `${progress}%` }} />
                </div>
                <p className="mt-2 text-right text-sm text-[rgb(var(--color-text-light))]">Step {step + 1} of {quizSteps.length}</p>
              </div>

              <div className="mb-8">
                <h3 className="mb-2 text-center text-[rgb(var(--color-navy))]" style={{ fontWeight: 400 }}>{currentStep.question}</h3>
                {currentStep.subtitle && <p className="text-center text-[rgb(var(--color-text-light))]">{currentStep.subtitle}</p>}
              </div>
                ...
                {currentStep.options.map((option) => (
                  ...
                  >
                    <span className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full transition-colors ${
                      selectedOption === option.value ? 'bg-[rgb(var(--color-green))] text-white' : 'bg-[rgb(var(--color-linen))] text-[rgb(var(--color-navy))]'
                    }`}>
                      {option.icon}
                    </span>
                    <span className="text-[rgb(var(--color-navy))]" style={{ fontWeight: 500 }}>{option.label}</span>
                  </button>
                ))}
              </div>

              <div className="flex items-center justify-between">
                <button onClick={handleBack} disabled={step === 0} className={`inline-flex items-center gap-2 text-sm transition-colors ${step === 0 ? 'cursor-not-allowed text-[rgb(var(--color-text-light))]/50' : 'text-[rgb(var(--color-navy))] hover:text-[rgb(var(--color-green))]'}`}>
                  <ArrowLeft className="h-4 w-4" />Back
                </button>
                
                <button onClick={handleNext} disabled={!selectedOption} className={`btn-pill ${selectedOption ? 'btn-pill-primary' : 'pointer-events-none opacity-50'}`}>
                  <span className="btn-text">{step === quizSteps.length - 1 ? 'See results' : 'Continue'}</span>
            ...
            <>
              <div className="mb-10 grid gap-4 sm:grid-cols-3">
                {results?.services.map((service) => (
                  <a key={service.link} href={service.link} className="group rounded-lg border border-[rgb(var(--border))]/50 bg-white p-6 text-center transition-all hover:border-[rgb(var(--color-green))] hover:shadow-md">
                    <span className="block text-[rgb(var(--color-navy))] transition-colors group-hover:text-[rgb(var(--color-green))]" style={{ fontWeight: 500 }}>{service.name}</span>
              ...
              <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
                {site === 'main' ? (
                  ...
                  <a href={contactUrl} className="btn-pill btn-pill-primary">
                    <span className="btn-text">{results?.cta}</span>
                    <span className="btn-arrow"><ArrowRight /></span>
                  </a>
                )}
                
                <button onClick={handleReset} className="inline-flex items-center gap-2 text-sm text-[rgb(var(--color-navy))] transition-colors hover:text-[rgb(var(--color-green))]">
                  <ArrowLeft className="h-4 w-4" />Take quiz again
                </button>
              </div>
            </>
          )}
