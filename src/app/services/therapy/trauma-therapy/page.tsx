import { Metadata } from 'next'
import Link from 'next/link'
import Image from 'next/image'
import { ArrowRight, ChevronDown } from 'lucide-react'
import { PageHero } from '@/components/sections/page-hero'
import { getRandomOfficeImage } from '@/lib/office-images'

export const metadata: Metadata = {
  title: 'Trauma Therapy on Long Island | Pathways Within',
  description: 'Heal from past experiences with trauma therapy at Pathways Within. Our specialists use Cognitive Processing Therapy and trauma-informed care on Long Island.',
}

const faqs = [
  { q: 'What is trauma therapy?', a: 'Trauma therapy is specialized treatment that helps you process and heal from traumatic experiences. It uses evidence-based approaches to help you feel safe, process difficult memories, and develop healthy coping strategies.' },
  { q: 'How do I know if I need trauma therapy?', a: 'Signs include intrusive memories, avoidance behaviors, heightened anxiety, difficulty sleeping, emotional numbness, or feeling like past events still affect your daily life.' },
  { q: 'Is trauma therapy different from regular therapy?', a: 'Yes, trauma therapy uses specialized techniques designed specifically for processing traumatic experiences. Our therapists are trained in trauma-informed approaches.' },
  { q: 'How long does trauma therapy take?', a: 'The duration varies based on your experiences and goals. Some people see significant progress in months, while others benefit from longer-term support.' },
]

const relatedServices = [
  { name: 'EMDR Therapy', href: '/services/therapy/emdr-therapy' },
  { name: 'Somatic Therapy', href: '/services/therapy/somatic-therapy' },
  { name: 'Individual Therapy', href: '/services/therapy/individual-therapy' },
  { name: 'Veterans & First Responders', href: '/services/therapy/veterans-first-responders' },
]

export default function TraumaTherapyPage() {
  return (
    <main>
      <PageHero
        eyebrow="Therapy Services"
        headline="Trauma therapy"
        subheadline="Healing from difficult experiences"
        body="Heal from past experiences with specialized approaches including Cognitive Processing Therapy and trauma-informed care. Our trained therapists create a safe space for your healing journey."
        variant="fullBleed"
        size="sm"
        image={{ alt: 'Peaceful healing environment with soft natural light', placeholder: true }}
      />

      <section className="border-t border-[rgb(var(--border))]/50 bg-[rgb(var(--color-cream))]">
        <div className="container-site py-20 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-20">
            <div>
              <p className="mb-4 text-sm uppercase tracking-widest text-[rgb(var(--color-green))]" style={{ fontFamily: 'var(--font-raleway), system-ui, sans-serif', fontWeight: 500 }}>About</p>
              <h2 className="text-[rgb(var(--color-navy))]">Understanding trauma</h2>
            </div>
            <div className="space-y-6">
              <p className="text-[rgb(var(--color-text-light))]" style={{ lineHeight: 1.8 }}>
                Trauma can result from a single event or ongoing experiences. It affects not just your mind but your whole body. Our trauma-informed approach recognizes this connection and works to heal both.
              </p>
              <p className="text-[rgb(var(--color-text-light))]" style={{ lineHeight: 1.8 }}>
                You don't have to keep carrying the weight of past experiences. With the right support, healing is possible.
              </p>
            </div>
          </div>
        </div>
      </section>

      <section className="border-t border-[rgb(var(--border))]/50 bg-white">
        <div className="container-site py-20 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-20">
            <div className="relative aspect-[4/3] w-full overflow-hidden rounded-lg">
              <Image
                src={getRandomOfficeImage()}
                alt="Trauma therapy at Pathways Within"
                fill
                className="object-cover"
                sizes="(max-width: 1024px) 100vw, 50vw"
              />
            </div>
            <div className="flex flex-col justify-center">
              <p className="mb-4 text-sm uppercase tracking-widest text-[rgb(var(--color-green))]" style={{ fontFamily: 'var(--font-raleway), system-ui, sans-serif', fontWeight: 500 }}>Our Approach</p>
              <h2 className="mb-6 text-[rgb(var(--color-navy))]">Evidence-based treatment</h2>
              <p className="mb-6 text-[rgb(var(--color-text-light))]" style={{ lineHeight: 1.8 }}>
                We use proven approaches to help you process trauma safely and effectively:
              </p>
              <ul className="space-y-3 text-[rgb(var(--color-text-light))]">
                <li>• Cognitive Processing Therapy (CPT)</li>
                <li>• Trauma-focused CBT</li>
                <li>• EMDR (Eye Movement Desensitization and Reprocessing)</li>
                <li>• Somatic experiencing</li>
                <li>• Mindfulness-based approaches</li>
              </ul>
            </div>
          </div>
        </div>
      </section>

      <section className="border-t border-[rgb(var(--border))]/50 bg-[rgb(var(--color-linen))]">
        <div className="container-site py-20 lg:py-28">
          <div className="mx-auto max-w-3xl">
            <h2 className="mb-12 text-center text-[rgb(var(--color-navy))]">Frequently asked questions</h2>
            <div className="space-y-0">
              {faqs.map((faq, idx) => (
                <details key={idx} className="group border-b border-[rgb(var(--border))]/50">
                  <summary className="flex cursor-pointer items-center justify-between py-6">
                    <span className="pr-8 text-[rgb(var(--color-navy))]" style={{ fontWeight: 500 }}>{faq.q}</span>
                    <ChevronDown className="h-5 w-5 flex-shrink-0 text-[rgb(var(--color-green))] transition-transform group-open:rotate-180" />
                  </summary>
                  <p className="pb-6 text-[rgb(var(--color-text-light))]" style={{ lineHeight: 1.8 }}>{faq.a}</p>
                </details>
              ))}
            </div>
          </div>
        </div>
      </section>

      <section className="border-t border-[rgb(var(--border))]/50 bg-white">
        <div className="container-site py-20 lg:py-28">
          <h2 className="mb-12 text-center text-[rgb(var(--color-navy))]">Related services</h2>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {relatedServices.map((service) => (
              <Link key={service.href} href={service.href} className="group rounded-lg border border-[rgb(var(--border))]/50 bg-white p-6 text-center transition-all hover:border-[rgb(var(--color-green))] hover:shadow-md">
                <span className="text-[rgb(var(--color-navy))] transition-colors group-hover:text-[rgb(var(--color-green))]" style={{ fontWeight: 500 }}>{service.name}</span>
              </Link>
            ))}
          </div>
        </div>
      </section>

      <section className="border-t border-[rgb(var(--border))]/50 bg-gradient-navy">
        <div className="container-site py-20 lg:py-28">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="mb-6 text-white">Ready to begin healing?</h2>
            <p className="mb-10 text-lg text-white/70" style={{ lineHeight: 1.8 }}>Contact us to learn more about trauma therapy and take the first step toward healing.</p>
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              <Link href="/contact" className="btn-pill btn-pill-white"><span className="btn-text">Contact us</span><span className="btn-arrow"><ArrowRight /></span></Link>
              <a href="tel:+16313713825" className="inline-flex items-center gap-2 text-white/80 transition-colors hover:text-white" style={{ fontFamily: 'var(--font-raleway), system-ui, sans-serif', fontWeight: 500 }}>Or call (631) 371-3825</a>
            </div>
          </div>
        </div>
      </section>
    </main>
  )
}
